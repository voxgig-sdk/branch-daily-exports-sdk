...
class BranchDailyExportsMakeError
{
    public static function call(?BranchDailyExportsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BranchDailyExportsContext([], null);
        }
        $op = $ctx->op ?? new BranchDailyExportsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BranchDailyExportsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BranchDailyExportsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new BranchDailyExportsError('', $msg, $ctx);
        ...
        if ($err instanceof BranchDailyExportsError) {
