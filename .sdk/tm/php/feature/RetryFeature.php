...
// BranchDailyExports SDK retry feature
...
class BranchDailyExportsRetryFeature extends BranchDailyExportsBaseFeature
{
    ...
    public function init(BranchDailyExportsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BranchDailyExportsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BranchDailyExportsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BranchDailyExportsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
