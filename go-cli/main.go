// Command BranchDailyExports-cli is a CLI + REPL for the BranchDailyExports SDK,
...
const prompt = "branch-daily-exports"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "export"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BRANCH_DAILY_EXPORTS_APIKEY (api key), BRANCH_DAILY_EXPORTS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BRANCH_DAILY_EXPORTS_APIKEY carries the API key and
	// BRANCH_DAILY_EXPORTS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BRANCH_DAILY_EXPORTS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BRANCH_DAILY_EXPORTS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBranchDailyExportsSDK(opts)
