# branch-daily-exports-cli

boru-driven command-line client **and** interactive REPL for the BranchDailyExports
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/branch-daily-exports-cli)
make build

# 2. See usage (words, entities, env vars)
./branch-daily-exports-cli --help
...
BRANCH_DAILY_EXPORTS_BASE=https://api.example.com ./branch-daily-exports-cli --help

# 6. No arguments -> interactive REPL
./branch-daily-exports-cli
branch-daily-exports> /help
branch-daily-exports> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/branch-daily-exports-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/branch-daily-exports-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `branch-daily-exports>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/branch-daily-exports-cli`. |
...
(`./branch-daily-exports-cli <expr>`) and interactively (the REPL), and expressions compose the
