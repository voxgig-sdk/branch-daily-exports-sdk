# branch-daily-exports-mcp

[MCP](https://modelcontextprotocol.io) server exposing the BranchDailyExports SDK as
two agent tools — `branch-daily-exports_list` and `branch-daily-exports_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/branch-daily-exports-mcp)
make build

# 2. Provide credentials via the environment
export BRANCH_DAILY_EXPORTS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user branch-daily-exports \
  -- /absolute/path/to/branch-daily-exports-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./branch-daily-exports-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// branch-daily-exports_list: first page of records
{ "entity": "export" }
{ "entity": "export", "query": { } }

// branch-daily-exports_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/branch-daily-exports-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user branch-daily-exports \
     -- "$PWD"/dist/*/branch-daily-exports-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `branch-daily-exports_list` and `branch-daily-exports_load` tools now appear
   in new sessions. Ask the agent to *"list export using branch-daily-exports"*
   and it calls `branch-daily-exports_list` with `{"entity":"export"}`.
...
```sh
export BRANCH_DAILY_EXPORTS_APIKEY=sk_live_xxx            # API key
export BRANCH_DAILY_EXPORTS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `branch-daily-exports_list` tool
...
### Call the `branch-daily-exports_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `branch-daily-exports_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `branch-daily-exports_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BRANCH_DAILY_EXPORTS_APIKEY` | API key sent with every request. |
| `BRANCH_DAILY_EXPORTS_BASE` | Optional override of the API base URL. |
...
```sh
./branch-daily-exports-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"branch-daily-exports_load","arguments":{"entity":"export","query":{"id":1}}}}'
```
