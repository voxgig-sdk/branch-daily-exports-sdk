...
// BranchDailyExports SDK Export entity
...
class ExportEntity
{
    ...
    {
        if ($args) {
            $this->_data = BranchDailyExportsHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = BranchDailyExportsHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws BranchDailyExportsError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = BranchDailyExportsHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
