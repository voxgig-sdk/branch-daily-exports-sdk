...
// BranchDailyExports SDK feature test
//
...
class FtCtrl extends BranchDailyExportsControl
{
...
class FtClient
{
    ...
    private BranchDailyExportsUtility $utility;

    public function __construct(BranchDailyExportsUtility $utility, array $options)
    {
    ...
    public function get_utility(): BranchDailyExportsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BranchDailyExportsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new BranchDailyExportsUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (BranchDailyExportsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BranchDailyExportsConfig::make_feature($name);
    ...
    private static function buildUrl(BranchDailyExportsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BranchDailyExportsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BranchDailyExportsResponse($fetched);
        }
    ...
    private function fail(BranchDailyExportsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BranchDailyExportsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
