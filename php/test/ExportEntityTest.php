...
class ExportEntityTest extends TestCase
{
    ...
    {
        $testsdk = BranchDailyExportsSDK::test(null, null);
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BRANCH_DAILY_EXPORTS_TEST_EXPORT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function export_basic_setup($extra)
{
    ...
    $client = BranchDailyExportsSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("BRANCH_DAILY_EXPORTS_TEST_EXPORT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["BRANCH_DAILY_EXPORTS_TEST_EXPORT_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["BRANCH_DAILY_EXPORTS_TEST_LIVE"] === "TRUE") {
        ...
        $client = new BranchDailyExportsSDK(Helpers::to_map($merged_opts) ?? []);
    }

    $live = $env["BRANCH_DAILY_EXPORTS_TEST_LIVE"] === "TRUE";
    return [
        "client" => $client,
        "data" => $entity_data,
        "idmap" => $idmap_resolved,
        "env" => $env,
        "explain" => $env["BRANCH_DAILY_EXPORTS_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
