...
// BranchDailyExports SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class BranchDailyExportsTestFeature extends BranchDailyExportsBaseFeature
{
    ...
    public function init(BranchDailyExportsContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (BranchDailyExportsContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = BranchDailyExportsParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (BranchDailyExportsContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(BranchDailyExportsContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = BranchDailyExportsParam::call($ctx, $k);
