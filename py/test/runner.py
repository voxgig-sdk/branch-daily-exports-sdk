# BranchDailyExports SDK test runner

from __future__ import annotations
import os
import json

from branchdailyexports_sdk.utility.voxgig_struct import voxgig_struct as vs


class BranchDailyExportsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BranchDailyExportsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BranchDailyExportsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BranchDailyExportsTestRunner.getenv("BRANCH_DAILY_EXPORTS_TEST_LIVE")
        override = BranchDailyExportsTestRunner.getenv("BRANCH_DAILY_EXPORTS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BranchDailyExportsTestRunner.getenv(key)
        ...
        explain = BranchDailyExportsTestRunner.getenv("BRANCH_DAILY_EXPORTS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["BRANCH_DAILY_EXPORTS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if BranchDailyExportsTestRunner._test_control is not None:
            return BranchDailyExportsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BranchDailyExportsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BranchDailyExportsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BranchDailyExportsTestRunner.load_test_control()
...
def load_env_local():
    BranchDailyExportsTestRunner.load_env_local()


def env_override(m):
    return BranchDailyExportsTestRunner.env_override(m)


def entity_data(v):
    return BranchDailyExportsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BranchDailyExportsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BranchDailyExportsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BranchDailyExportsTestRunner.load_test_control()


def live_client_options():
    return BranchDailyExportsTestRunner.live_client_options()


def live_delay_ms():
    return BranchDailyExportsTestRunner.live_delay_ms()
