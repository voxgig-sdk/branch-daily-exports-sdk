...
require("branch-daily-exports_types")
...
local BranchDailyExportsSDK = {}
BranchDailyExportsSDK.__index = BranchDailyExportsSDK
...
BranchDailyExportsSDK._make_feature = _make_feature


function BranchDailyExportsSDK.new(options)
  local self = setmetatable({}, BranchDailyExportsSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function BranchDailyExportsSDK:options_map()
...
function BranchDailyExportsSDK:get_utility()
  return Utility.copy(self._utility)
end


function BranchDailyExportsSDK:get_root_ctx()
  return self._rootctx
end


function BranchDailyExportsSDK:prepare(fetchargs)
...
function BranchDailyExportsSDK:direct(fetchargs)
...
function BranchDailyExportsSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function BranchDailyExportsSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "BranchDailyExportsSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function BranchDailyExportsSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function BranchDailyExportsSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "BranchDailyExportsSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function BranchDailyExportsSDK:Export(data)
...
function BranchDailyExportsSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = BranchDailyExportsSDK.new(sdkopts)
  sdk.mode = "test"
