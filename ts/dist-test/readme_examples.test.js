...
const __1 = require("..");
const SDK_NAME = 'BranchDailyExportsSDK';
...
const TEST_SEED = { "entity": { "export": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+BranchDailyExportsSDK\b/.test(code) ||
        /\bBranchDailyExportsSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+BranchDailyExportsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/BranchDailyExportsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
